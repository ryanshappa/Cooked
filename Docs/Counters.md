...
| `CuttingCounter` | — (E places/picks up; **chopping requires the held Knife** — hover-tool aims the cut, LMB chops at that spot) | only items with a **CuttingRecipeSO** | Each chop **slices the ingredient's mesh for real** on the aimed plane (`Sliceable` → `MeshSlicingService`/EzySlice); the two pieces spawn loose on the board (physics-rested, re-cuttable). The board's slot empties after the first cut; loose pieces are targetable while resting on a board with a recipe for them. Recipe retag + evenness metric: `Docs/MeshSlicing.md` step 4. Bare-hand LMB no longer chops. |
...
**Sizing convention:** all counter prefabs match — visual child scaled **0.72** (KC visuals; the table's pack visual at 0.9), collider ≈ (1.07, 0.91, 1.05), surface at y ≈ **0.91** (lowered Aug 2026 for the small chef). Scale the *visual child*, never the prefab root (root scale would shrink items parented onto the counter). TrashBin is the exception — root-scaled, safe because nothing ever parents to it.
